...
class ModelChecker:
    ...
    def check_repository_file(self, entity_name: str, verbose: bool = False):
        ...
        if not os.path.exists(repository_path):
            if verbose:
                Printer().print_msg(f"Repository file '{
                    repository_file_name}' does not exist.", theme="error")
            return False
        return True
        ...
        with open(entity_path, "r") as f:
            ...
            for line in lines:
                if "class" in line and entity_class_name in line:
                    return True
        if verbose:
            Printer().print_msg(f"Entity class '{
                entity_class_name}' does not exist.", theme="error")
        return False
        ...
        with open(repository_path, "r") as f:
            ...
            for line in lines:
                if "class" in line and repository_class_name in line:
                    return True
        if verbose:
            Printer().print_msg(f"Repository class '{
                repository_class_name}' does not exist.", theme="error")
        return False
    ...
    def check_entity_and_repository(self, entity_name: str, verbose: bool = False):
        ...
        """
        return self.check_entity_file(entity_name, verbose) and\
