...
class CommandSuggestionService:
    ...
    def suggest_command_with_subcommand(
        self, args: List[str]
        ...
        group_suggestions = suggest_similar_commands(
            main_command, available_groups, threshold=3
        )
        ...
        if best_group in command_groups:
            ...
            best_subcommand = self._find_best_subcommand_match(
                potential_subcommand, available_subcommands
            )
    ...
    def _find_best_subcommand_match(
        self, target_subcommand: str, available_subcommands: List[str]
    ) -> Optional[str]:
        """
        ...
        # 1. Prefix matching
        prefix_matches = [
            cmd for cmd in available_subcommands if cmd.lower().startswith(target_lower)
        ]
        if prefix_matches:
        ...
        inclusion_matches = [
            cmd for cmd in available_subcommands if target_lower in cmd.lower()
        ]
        ...
        suggestions = suggest_similar_commands(
            target_subcommand, available_subcommands, threshold=3
        )
        if suggestions:
