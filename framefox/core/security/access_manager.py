...
class AccessManager:
    ...
    def is_allowed(self, user_roles: List[str], required_roles: List[str]) -> bool:
        ...
        self.logger.debug(f"User roles: {user_roles}, Required roles: {required_roles}")
        self.logger.debug(f"The user {'has' if has_role else 'does not have'} the required roles.")
        return has_role
